...
Base.@kwdef struct Params
    ...
    savefile::String = "run_adaptive_implicit_coupled_uw_iter_uwd.jls"
end
...
function get_history(u, w, p::Params)
    ax = DfDx(u,p) .- DfDx(w,p)
    history = zeros(length(ax))
    for i in eachindex(ax)
        ...
        # use ⟨.⟩_+
        history[i] = positive(psi_plus - p.psi_crit)
    end
    return history
end
...
function solve_uwd_step(u_old, w_old, d_old, H_old, T, p::Params)
    N  = p.N
    dx = p.dx
    idx = 1/dx
    n = p.n
    ...
    for iter in 1:p.max_iter
        ...
        # use new sol in d calc
        u_new = sol[1:N]
        w_new = sol[N+1:end]

        # get history
        H_new = get_history(u_new,w_new,p)
        H = max.(H_old, H_new)
        ...
        for i in 2:N-1
            MM[i, i-1] = -l^2 / dx^2
            MM[i, i]   = 1 + 2*l^2/dx^2 + 2*C3*l*H[i]
            MM[i, i+1] = -l^2 / dx^2
            f[i]      = 2*C3*l*H[i]
        end

        # Neumann BC at i=1 (ghost node)
        MM[1,1] = 1 + 2*l^2/dx^2 + 2*C3*l*H[1]
        MM[1,2] = -2*l^2/dx^2
        f[1]   = 2*C3*l*H[1]

        # Neumann BC at i=N (ghost node)
        MM[N,N]   = 1 + 2*l^2/dx^2 + 2*C3*l*H[N]
        MM[N,N-1] = -2*l^2/dx^2
        f[N]     = 2*C3*l*H[N]
        d_new = MM \ f

        res = maximum(abs.(u_new-u)) + maximum(abs.(w_new-w)) + maximum(abs.(d_new-d))
...
function run_simulation(p::Params)
    ...
    while t < p.tf
        ...
        u, w, d, H, dt, res = solve_uwd_step(u,w,d,H,T,p)
